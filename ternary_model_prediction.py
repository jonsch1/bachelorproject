...
from ternary_generator import make_ternary_complex
from ternary_generator import args, read_atom_file
    ...
    with open(rmsd_filename, "w") as fwr:
        ...
        for d in decoys:
            ...
            for l in linkers:
                ...
                print("rmsd:",rmsd)
                #improve efficiency: check if rmsd is far away from cutoff:
                rmsd_tracker.append(rmsd)
                rmsd_min = min(rmsd_tracker)

                if len(rmsd_tracker) > 9 and rmsd_min > 1.5:
                   print("rmsd min over ", len(rmsd_tracker), " is greater 1.5:", rmsd_min)
                   break

                # write structure if rmsd between aligned decoy and linker
                # pass the cut off threshold
                if rmsd < cutoff:
                    print("add 1 more to the bank, we gonna be rich")
